        ...
        for file in uploaded_files:
            try:
                ...
                if result:
                    ...
                    with col1:
                        st.image(result['annotated_image'], caption=f"Analysis: {file.name}", 
                                use_column_width=True)
                    
                    with col2:
                        st.markdown(f"**Overall Class:** {result['class']}")
                        st.markdown(f"**Confidence:** {result['confidence']:.1f}%")
                        
                        if result['detected_objects']:
                            st.markdown("**Detected Objects:**")
                            for i, obj in enumerate(result['detected_objects'], 1):
                                st.markdown(f"- Object {i}: {obj['class']} ({obj['confidence']:.1f}%)")
